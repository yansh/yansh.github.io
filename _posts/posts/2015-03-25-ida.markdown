...
The evolution of smart devices has pushed the web design forward towards a responsive design. It is a design approach that is programmed to adjust (respond) to the projected xenvironment. The Web has been really good at adopting the responsive design. Responsive __Web__ design  uses existing CSS techniques to allow content to be *responsive* (and cadjust) accordingly to any medium  used to access it, e.g., mobile phone browser, smart-watch, different size tv.
...
I think the problem  is twofold: firstly, both the designer and the software engineer don't have the tools to translate these "insights" on to the app/web/service itsel.  Secondly, figuring out the insight and reacting to it are independent processes. The insight is retrieved by scientist, applying sophisticated algorithms, while software is written and websites are built by software engineers and designers. It's very static, driven by templates-approach and does not really reflect the spirit of the "response design" framework. I think, a main reason to why responsive design works (largely) is because we provided the designer the tools/frameworks to incorporate inside the product. 
