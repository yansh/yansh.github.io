...
I think the problem  is twofold: firstly, both the designer and the software engineer don't have the tools to translate these "insights" on to the app/web/service itself. Secondly, figuring out the insight and reacting to it are independent processes. The insight is retrieved by scientist, applying sophisticated algorithms, while software is written and websites are built by software engineers and designer. It's very static, driven by templates-approach and does not really reflect the spirit of the "response design" framework. I think, a main reason to why responsive design works (largely) is because we provided the designer the tools/frameworks to incorporate inside the product. 
